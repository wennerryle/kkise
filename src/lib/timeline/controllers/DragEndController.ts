...
export class DragEndController {
	...
	constructor(timelineCtx: TimelineContext) {
		this.trackRepo = timelineCtx.trackRepository;
		this.intervalRepo = timelineCtx.intervalRepository;
	}
	...
	onDragEnd: DnDEvents['onDragEnd'] = (event, manager) => {
		...
		if (sourceData.tag === 'interval' && targetData.tag === 'track') {
			this.handleIntervalsMoveAcrossTracks(sourceData, targetData, sourceId);
			return;
		}
	};
	...
	private readonly handleIntervalsMoveAcrossTracks = (
		...
		if (detectIntervalToTracksMovingCollision(this.intervalRepo, targetTrack, checkedInterval)) {
			return;
		}

		sourceTrack.intervals = sourceTrack.intervals.filter((it) => it !== intervalId);
